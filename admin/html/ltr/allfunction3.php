...
function fetch_personal_message(){
  ...
  global $con;
  $refer_id=$_POST['refer_id'];
  $self_id=$_SESSION['sender_id'];
  $data="";

  $q = $con->query("select distinct(message),datetime,sender_id ,concat(firstname,' ',lastname) as fullname from group_chat JOIN registration  where  type='group' and sender_id=id");
  //echo "select distinct(message),datetime,sender_id from group_chat  where receiver_id='$refer_id' and sender_id='$self_id' or receiver_id='$self_id' and sender_id='$refer_id' and type='group'";
...
function personal_msg_insert(){
    ...
    $q=$con->query("insert into group_chat(sender_id,receiver_id,datetime,message,type) values('$sender','$receiver_id','$datetime','$message','group')");
}
 ...
 $result = mysqli_query($con,$sql);
      ...
      if(mysqli_num_rows($result)>0)
      {
        ...
        {             
          ...
          $output .=');';
          $output .='>';
          $output .='<img width="40px" height="40px" src="../../assets/images/logo.png"';
