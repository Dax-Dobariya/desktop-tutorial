...
function personal_msg_insert(){
    ...
    // for($i=0;$i<count($refer_id);$i++){

    //   $q=$con->query("select refer_id FROM project_group WHERE pr_refer_id IN ('".$refer_id."')"); 
    //   if(mysqli_num_rows($q)>0){
    //   while($row = mysqli_fetch_array($q))
    //   {
    //       $receiver_id = $row['refer_id'];
    //   }
    // }
...
function project_personal_search(){
 ...
 $sql = "SELECT * FROM registration where id!='$self_id' and occupation = 'admin' GROUP BY firstname";

//  select refer_id,firstname,lastname,p_profile_pic FROM personal_detail  where refer_id!='$self_id' GROUP BY firstname
